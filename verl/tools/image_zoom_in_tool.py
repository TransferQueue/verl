...
class ImageZoomInTool(BaseTool):
    ...
    async def create(self, instance_id: Optional[str] = None, **kwargs) -> tuple[str, ToolResponse]:
        ...
        if self.tq_client is not None:
            ...
            image = await get_multi_modal_data(self.tq_client, image, "image")

            if isinstance(image, list):
                # TODO(yixiang) Maybe a better solution for multi-turn tool calling.
                # If len(image) > 1, it means that there are new created images by tool.
                # Here we only take the original image as input for each turn.
                image = image[0]
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        if image_batch_meta and cropped_image:
            new_img_batch_meta = await self.tq_client.async_put(
                data=TensorDict({"image": [cropped_image]}, batch_size=1),
                partition_id=image_batch_meta[0].samples[0].partition_id
            )
            return (
                ToolResponse(
