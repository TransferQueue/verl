...
try:
    from verl.utils.transferqueue_utils import (_run_async_in_temp_loop,
                                                get_routed_experts_data,
                                                is_routed_experts_batchmeta)
except ImportError:
    # Fallback for when transfer_queue is not available
    _run_async_in_temp_loop = None
    get_routed_experts_data = None
    is_routed_experts_batchmeta = None
...
class MegatronPPOActor(BasePPOActor):
    def __init__(
        ...
        Args:
            ...
            tq_client (AsyncTransferQueueClient, optional): TransferQueue client for reading routed_experts data.
        ...
        self.tq_client = tq_client  # Store TQ client reference
        self.use_torch_profiler = self.config.profiler.get("tool") == "torch"
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        if recompute_old_log_prob:
            # In R3 mode, routed_experts comes from rollout and needs to be prepared from non_tensor_batch
            # In R2 mode, routed_experts is generated during compute_log_prob, no preparation needed
            if self.enable_routing_replay and self.config.router_replay.mode == "R3":
                self._prepare_routed_experts_for_batch(data, "routed_experts")
    ...
    def forward_backward_batch(
        ...
        # In R3 mode, routed_experts comes from rollout and may be in non_tensor_batch
        # In R2 mode, routed_experts is already in batch (generated by compute_log_prob)
        if self.enable_routing_replay and self.config.router_replay.mode == "R3":
            self._prepare_routed_experts_for_batch(mini_batch, "routed_experts")
        ...
        def forward_step(batch_iter, model, return_schedule_plan: bool = False):
            ...
            if RouterReplayHelper.is_replay_forward_action(self.tf_config, vp_rank):
                # Routed experts is already converted to tensor in forward_backward_batch
    ...
    def _prepare_routed_experts_for_batch(self, data: DataProto, key: str) -> None:
        """
        Prepare routed_experts data from non_tensor_batch and move it to batch.
        If TransferQueue is enabled, read actual data from TQ. Otherwise, stack existing tensors.

        Args:
            data: DataProto containing the data
            key: The key for routed_experts (typically "routed_experts")
        """
        if key not in data.non_tensor_batch.keys():
            return

        # Get routed_experts references (BatchMeta or tensor for each sample)
        routed_experts_refs = data.non_tensor_batch[key]

        # Check if it's a BatchMeta structure with safety checks
        if (self.tq_client is not None and
            get_routed_experts_data is not None and
            is_routed_experts_batchmeta is not None and
            routed_experts_refs and  # Check if list is not empty
            routed_experts_refs[0] is not None and  # Check if first element is not None
            is_routed_experts_batchmeta(routed_experts_refs[0])):
            # Read actual data from TQ for all samples
            all_routed_experts = []
            for ref in routed_experts_refs:
                tensor = _run_async_in_temp_loop(
                    get_routed_experts_data, self.tq_client, ref
                )
                all_routed_experts.append(tensor)
            # Stack into batch tensor [bs, seq_len, layer_num, topk]
            data.batch[key] = torch.stack(all_routed_experts)
        else:
            # Already tensors, stack them with proper type handling
            stacked_tensors = []
            for ref in routed_experts_refs:
                if isinstance(ref, torch.Tensor):
                    stacked_tensors.append(ref)
                elif hasattr(ref, 'shape'):  # numpy array or similar
                    stacked_tensors.append(torch.from_numpy(ref))
                else:
                    # Try to convert other types to tensor
                    stacked_tensors.append(torch.tensor(ref))
            data.batch[key] = torch.stack(stacked_tensors)
