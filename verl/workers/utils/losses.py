...
def _slice_response_from_unpad_output(tensor: torch.Tensor, data: TensorDict, include_prompt:bool=True) -> torch.Tensor:
    """Slice response from unpad model output, considering variables with response_length or prompt+response_mask length.
    ...
    if include_prompt:
        ...
        response_list = []
        for resp_len, nt in zip(response_lens, tensor):
